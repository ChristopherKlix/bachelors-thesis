...
class PlantEnergyAggregations:
    ...
    materialized views (MVs) and views related to plant energy data.
    ...
    **Example:**::
        >>> agg = PlantEnergyAggregations()
        >>> agg.start_auto_update()
    """

    PLANT_ID: str = 'main_data_2_id'
    ...
    async def rebuild(self, confirm: bool=False) -> None:
        """
        ...
        # Get the earliest record in ``plant_energy``
        ...
        # Create the ``plant_energy_day_%`` and ``plant_energy_month_%``
        # for each year (and month)

        for year in range(start_year, now.year+1):
            for month in range(1, 12+1):
                await self.update_plant_energy_hour(year, month)
                await self.update_plant_energy_day(year, month)

        await self._create_or_replace_plant_energy_hour_view()
        await self._create_or_replace_plant_energy_day_view()

        for year in range(start_year, now.year+1):
            await self.update_plant_energy_month(year)

        # Finally, create the VIEWs that combine all MVs
        # ``plant_energy_year``: just in time aggregation of VIEW ``plant_energy_month``
        # ``plant_energy_month``: union of all MVs ``plant_energy_month_%``
        # ``plant_energy_day``: union of all MVs ``plant_energy_day_%``

        await self._create_or_replace_plant_energy_month_view()

        await self._create_or_replace_plant_energy_year_view()
    ...
    async def clear(self, confirm: bool=False) -> None:
        ...
        await self._drop_plant_energy_year_view()

        await self._drop_plant_energy_month_view()

        await self._drop_plant_energy_day_view()

        await self._drop_plant_energy_hour_view()
        ...
        def run_query(aggregation: str) -> list[str]:
            with self._conn.cursor() as cur:
                cur.execute(sql_query, {'mv_name': f'plant_energy_{aggregation}_%'})
                mv_list: list[str] = [row[0] for row in cur.fetchall()]

                return mv_list
        
        ################################
        # Get a list of all MATERIALIZED VIEWs: plant_energy_month_XXXX
        ################################
            
        mv_list: list[str] = await asyncio.to_thread(run_query, aggregation='month')

        for mv_name in mv_list:
            await self._drop_plant_energy_mv(mv_name)
        
        ################################
        # Get a list of all MATERIALIZED VIEWs: plant_energy_day_XXXX_XX
        ################################
        ...
        ################################
        # Get a list of all MATERIALIZED VIEWs: plant_energy_hour_XXXX_XX
        ################################
    ...
    async def start_auto_update(
        ...
        await self._start_auto_update_plant_energy_hour(
            include_last_month=rule_hour_include_last_month
        )
        print(f'Plant Energy Aggregation Hour: {pd.Timestamp.now() - _t}')

        # Daily Energy
        _t = pd.Timestamp.now()
        await self._start_auto_update_plant_energy_day(
            include_last_month=rule_day_include_last_month
        )
        print(f'Plant Energy Aggregation Day: {pd.Timestamp.now() - _t}')

        # Monthly Energy
        _t = pd.Timestamp.now()
        await self._start_auto_update_plant_energy_month(
            include_last_year=rule_month_include_last_year
        )
        print(f'Plant Energy Aggregation Month: {pd.Timestamp.now() - _t}')

        # Yearly Energy
        _t = pd.Timestamp.now()
        await self._start_auto_update_plant_energy_year()
        print(f'Plant Energy Aggregation Year: {pd.Timestamp.now() - _t}')

    # AUTO UPDATE

    async def _start_auto_update_plant_energy_hour(
            self,
        ...
        matview_pattern: str = f'plant_energy_hour_{now.year}_%'
        ...
        if len(matviews) == 0:
            ...
            for i in range(1, 13):
                ...
                await self.update_plant_energy_hour(year=now.year, month=i)

            print(f'Creating view plant_energy_hour...')
            # CREATE OR REPLACE VIEW ``plant_energy_hour``
            await self._create_or_replace_plant_energy_hour_view()
        elif len(matviews) != 12:
            raise RuntimeError(
                f'An error occured while checking database integrity. '
                f'12 MATERIALIZED VIEWs for plant_energy_hour_{now.year} expected.'
                f'{len(matviews)} MATERIALIZED VIEWs for plant_energy_hour_{now.year} exist.'
                f'Check the MATERIALIZED VIEWs for plant_energy_hour_{now.year}.'
            )
        else:
            ...
            await self.update_plant_energy_hour(year=now.year, month=now.month)
        ...
        if include_last_month:
            ...
            await self.update_plant_energy_hour(
                year=last_month.year,
                month=last_month.month
            )

    async def _start_auto_update_plant_energy_day(
            self,
            include_last_month: bool=False
        ) -> None:
        print(f'#' * 64)
        ...
        matview_pattern: str = f'plant_energy_day_{now.year}_%'
        ...
        if len(matviews) == 0:
            ...
            for i in range(1, 13):
                print(f'Creating matview for {now.year}-{i:02}.')
                await self.update_plant_energy_day(year=now.year, month=i)

            print(f'Creating view plant_energy_day...')
            # CREATE OR REPLACE VIEW ``plant_energy_day``
            await self._create_or_replace_plant_energy_day_view()
        elif len(matviews) != 12:
            raise RuntimeError(
                f'An error occured while checking database integrity. '
                f'12 MATERIALIZED VIEWs for plant_energy_day_{now.year} expected.'
                f'{len(matviews)} MATERIALIZED VIEWs for plant_energy_day_{now.year} exist.'
                f'Check the MATERIALIZED VIEWs for plant_energy_day_{now.year}.'
            )
        else:
            print(f'Matviews already exist.')
            print(f'Refreshing matview for {now.year}-{now.month:02}.')
            # REFRESH MATERIALIZED VIEW for the current month
            await self.update_plant_energy_day(year=now.year, month=now.month)

        if include_last_month:
            start_ts, _ = self._get_range_from_interval('last_month')
            last_month: int = pd.Timestamp(start_ts).tz_convert('Europe/Berlin')

            await self.update_plant_energy_day(
                year=last_month.year,
                month=last_month.month
            )

    async def _start_auto_update_plant_energy_month(
            self,
        ...
        matview_pattern: str = f'plant_energy_month_{now.year}'
        ...
        await self.update_plant_energy_month(year=now.year)

        if len(matviews) == 0:
            print(f'Since the matview didn\'t exist we need to create or replace the view plant_energy_month')
            # CREATE OR REPLACE VIEW ``plant_energy_month``
            await self._create_or_replace_plant_energy_month_view()
        ...
        if include_last_year:
            ...
            await self.update_plant_energy_month(year=last_year.year)

    async def _start_auto_update_plant_energy_year(self):
        print(f'#' * 64)
        print(f'Auto Update - Year')
        print(f'#' * 64)
        print(f'Create or replace view for plant_energy_year.')
        await self._create_or_replace_plant_energy_year_view()

    # CREATE or REFRESH

    async def update_plant_energy_hour(self, year: int, month: int) -> None:
        AGGREGATION: str = 'hour'

        if not (1 <= month <= 12):
            raise ValueError(f'month must be between 1 and 12, got {month}')
        
        sql_query: sql.Composable = self._get_plant_energy_hour_sql(year, month)

        partition: str = f'{year}_{month:02}'

        mv_name: str = f'plant_energy_{AGGREGATION}_{partition}'

        await self._update_plant_energy_mv(
            mv_name=mv_name,
            sql_select=sql_query
        )

    async def update_plant_energy_day(self, year: int, month: int) -> None:
        AGGREGATION: str = 'day'

        if not (1 <= month <= 12):
            raise ValueError(f'month must be between 1 and 12, got {month}')
        
        sql_query: sql.Composable = self._get_plant_energy_day_sql(year, month)
    ...
    async def update_plant_energy_month(self, year: int) -> None:
        AGGREGATION: str = 'month'

        sql_query: sql.Composable = self._get_plant_energy_month_sql(year)
    ...
    async def _drop_plant_energy_hour(self, year: int, month: int) -> None:
        AGGREGATION: str = 'hour'
    ...
    async def _drop_plant_energy_day(self, year: int, month: int) -> None:
        AGGREGATION: str = 'day'
    ...
    async def _drop_plant_energy_month(self, year: int) -> None:
        AGGREGATION: str = 'month'
    ...
    def _get_plant_energy_hour_sql(self, year: int, month: int) -> sql.Composable:
        ...
        sql_query: sql.Composable = sql.SQL ('''
            ...
            data AS (
                ...
                FROM {plant_energy} pe, period p
                WHERE pe.start_ts >= p.start_ts
                AND pe.start_ts <  p.end_ts
                GROUP BY {plant_id}, hour, start_ts_hour
                ORDER BY {plant_id}, hour, start_ts_hour
            )
            SELECT
                {plant_id},
        ...
        ''').format(
            ...
            plant_id=sql.Identifier(self.PLANT_ID),
            plant_energy=sql.Identifier('plant_energy')
        )

        return sql_query
    
    def _get_plant_energy_day_sql(self, year: int, month: int) -> sql.Composable:
        start_ts: pd.Timestamp = pd.Timestamp.fromisoformat(f'{year}-{month:02}-01T00:00')
        start_ts = start_ts.tz_localize('Europe/Berlin')
        end_ts: pd.Timestamp = start_ts + pd.offsets.DateOffset(months=1)
        ...
        sql_query: sql.Composable = sql.SQL ('''
            ...
            data AS (
                ...
                WHERE pe.start_ts >= p.start_ts
                AND pe.start_ts <  p.end_ts
                GROUP BY {plant_id}, day, start_ts_day
                ORDER BY {plant_id}, day, start_ts_day
            )
            SELECT
                {plant_id},
    ...
    def _get_plant_energy_month_sql(self, year: int) -> sql.Composable:
        ...
        # ex.: plant_energy_day_2025_10
        month_mvs: list[str] = [
            f'plant_energy_day_{year}_{i:02}' for i in range(1, 13)
        ]

        # Generate a SELECT query for each MV
        # ex.: SELECT * FROM plant_energy_day_2025_10
        ...
        # ex.:
        # SELECT * FROM plant_energy_day_2025_01 UNION ALL
        # SELECT * FROM plant_energy_day_2025_02 UNION ALL
        # ...
        # SELECT * FROM plant_energy_day_2025_12 ALL
        ...
        sql_query: sql.Composable = sql.SQL ('''
            ...
            data AS (
                SELECT
                    ...
                    SUM(energy_reactive_fundamental_supplied) AS energy_reactive_fundamental_supplied,
                    SUM(energy_reactive_fundamental_consumed) AS energy_reactive_fundamental_consumed
                FROM {plant_energy_day_year} pe, period p
                WHERE pe.start_ts >= p.start_ts
                AND pe.start_ts <  p.end_ts
                GROUP BY {plant_id}, month, start_ts_month
                ORDER BY {plant_id}, month, start_ts_month
            )
            SELECT
                {plant_id},
        ...
        ''').format(
            start_ts=sql.SQL('{}').format(sql.Literal(start_ts)),
            end_ts=sql.SQL('{}').format(sql.Literal(end_ts)),
            plant_energy_day_year=sql.Identifier(f'plant_energy_day_{year}'),
            union_sql=union_sql,
            plant_id=sql.Identifier(self.PLANT_ID)
        )
    ...
    def _get_plant_energy_year_sql(self) -> sql.Composable:
        sql_select: sql.Composable = sql.SQL('''
            WITH data AS (
                SELECT
                    ...
                    SUM(energy_reactive_fundamental_supplied) AS energy_reactive_fundamental_supplied,
                    SUM(energy_reactive_fundamental_consumed) AS energy_reactive_fundamental_consumed
                FROM {plant_energy_month} pe
                GROUP BY {plant_id}, year, start_ts_year
                ORDER BY {plant_id}, year, start_ts_year
            )
            SELECT
                {plant_id},
            ...
            FROM data
        ''').format(
            plant_energy_month=sql.Identifier('plant_energy_month'),
            plant_id=sql.Identifier(self.PLANT_ID)
        )
    ...
    async def _update_plant_energy_mv( 
            self, mv_name: str,
        ...
        sql_idx: sql.Composable = sql.SQL(
            ...
            idx_name=sql.Identifier(f'idx_{mv_name}_{self.PLANT_ID}_start_ts'),
            mv_name=sql.Identifier(mv_name),
            cols=sql.SQL(', ').join(map(sql.Identifier, [self.PLANT_ID, 'start_ts']))
        )
    ...
    async def _drop_plant_energy_mv(self, mv_name: str) -> None:
        sql_query: sql.Composable = sql.SQL(
            'DROP MATERIALIZED VIEW IF EXISTS {mv_name}'
        ).format(
            mv_name=sql.Identifier(mv_name)
    ...
    async def _create_or_replace_plant_energy_hour_view(self) -> None:
        AGGREGATION: str = 'hour'
        view_name: str = f'plant_energy_{AGGREGATION}'

        await self._create_or_replace_plant_energy_view(view_name)
    
    async def _create_or_replace_plant_energy_day_view(self) -> None:
        AGGREGATION: str = 'day'
        view_name: str = f'plant_energy_{AGGREGATION}'

        await self._create_or_replace_plant_energy_view(view_name)

    async def _create_or_replace_plant_energy_month_view(self) -> None:
        AGGREGATION: str = 'month'
        view_name: str = f'plant_energy_{AGGREGATION}'

        await self._create_or_replace_plant_energy_view(view_name)

    async def _create_or_replace_plant_energy_year_view(self) -> None:
        AGGREGATION: str = 'year'
        view_name: str = f'plant_energy_{AGGREGATION}'

        sql_select: sql.Composable = self._get_plant_energy_year_sql()
    ...
    async def _drop_plant_energy_hour_view(self) -> None:
        AGGREGATION: str = 'hour'
        view_name: str = f'plant_energy_{AGGREGATION}'

        await self._drop_plant_energy_view(view_name)

    async def _drop_plant_energy_day_view(self) -> None:
        AGGREGATION: str = 'day'
        view_name: str = f'plant_energy_{AGGREGATION}'

        await self._drop_plant_energy_view(view_name)

    async def _drop_plant_energy_month_view(self) -> None:
        AGGREGATION: str = 'month'
        view_name: str = f'plant_energy_{AGGREGATION}'

        await self._drop_plant_energy_view(view_name)

    async def _drop_plant_energy_year_view(self) -> None:
        AGGREGATION: str = 'year'
        view_name: str = f'plant_energy_{AGGREGATION}'
    ...
    async def _create_or_replace_plant_energy_view(self, view_name: str) -> None:
    ...
    async def _drop_plant_energy_view(self, view_name: str) -> None:
        sql_query: sql.Composable = sql.SQL(
